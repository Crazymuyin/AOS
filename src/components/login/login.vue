<template>
  <div class="login">
    <!--<vue-particles color="#00fdfe" :particleOpacity="0.7" :particlesNumber="40" shapeType="circle" :particleSize="4" linesColor="#00fdfe" :linesWidth="2" :lineLinked="true" :lineOpacity="0.4" :linesDistance="150" :moveSpeed="2" :hoverEffect="true" hoverMode="grab" :clickEffect="false" clickMode="push" class="particle">-->
    <!--</vue-particles>-->
    <div class="loginMap">
      <img class="earthVague" src="../../assets/login/quanquan.png" alt="">
      <img class="earth" src="../../assets/login/icon/earthIcon.png" alt="">
      <img class="vague edu" src="../../assets/login/icon/edu.png" alt="">
      <img class="vague energy" src="../../assets/login/icon/energy.png" alt="">
      <img class="vague medical" src="../../assets/login/icon/Medical.png" alt="">
      <img class="vague railway" src="../../assets/login/icon/railway.png" alt="">
      <div class="ellipse ellipse1">
        <img src="../../assets/login/icon/light.png" alt="">
      </div>
    ...
    <div class="loginMain">
      <div class="login_cont">
        ...
        <div class="lable_cell">
          <input type="text" placeholder="请输入用户账号" autocomplete="OFF" name="username" :onchange="searchPassword(userInfo.username)" v-model="userInfo.username" @keyup="enterLogin($event)">
          <span class="icon icon_user"></span>
        </div>
        <div class="lable_cell" style="margin-bottom: 0">
          <input type="password" placeholder="请输入用户密码" autocomplete="OFF" name="password" @keyup="enterLogin($event)" v-model="userInfo.password">
          <span class="icon icon_pwd"></span>
        </div>
        <div class="mychecks">
          <label><input type="checkbox" value="1" id="checkbox" name="rememberPwd" :checked="userInfo.remember" @click="rememberToggle">记住密码</label>
          <div class="ForgotPassword">
            <a title="忘记密码？">忘记密码？</a>
          </div>
        </div>
        <input type="button" class="login_btn" value="登录" @click="loginIn" @keyup="enterLogin($event)">
      </div>
    </div>
    <div class="loginFoot">
      <ul class="clearfix">
        <li class="fn-left">{{copyRight.footerTxt}}</li>
        <li class="fn-left">
          <a :href="copyRight.footerHref" target="_blank">
            <img :src="copyRight.footerImg">{{copyRight.footerNum}}
          </a>
        </li>
      </ul>
    </div>
...
<script type="application/ecmascript">
  ...
  import {delCookie, getCookie, setCookie, setLocalStorage, getLocalStorage, delLocalStorage} from '../../common/js/cookie'
    ...
    methods: {
      ...
      loginIn: function() {
        if (this.userInfo.username === '' || this.userInfo.password === '') {
          this.$layer.msg('请输入账号或密码')
        } else {
          let params = {
            account: this.userInfo.username,
            password: this.userInfo.password
          }
          let url = this.$store.state.url + 'auth/login'
          this.$get(url, params).then(response => {
              if (response.status === 1) {
                this.rememberPassword(this.userInfo.username, this.userInfo.password)
                setTimeout(function () {
                  let dateTime = 1000 * 60 * 30
                  let jsonData = {
                    account: this.userInfo.username,
                    userId: response.data['userId'],
                    taskId: response.data['defaultTaskId'],
                    region: response.data['region'],
                    userName: response.data['userName'],
                    registerTime: response.data['registerTime'],
                    endTime: response.data['endTime'],
                    menuAuth: response.data['perms'],
                    industry: response.data['industry']
                  }
                  setLocalStorage('userInfo', JSON.stringify(jsonData))
                  setCookie('auth', response.data.auth, dateTime)
      ...
      rememberToggle: function() {
        this.userInfo.remember = !this.userInfo.remember
      },
      rememberPassword: function (username, password) {
        username = Base64.encode(username)
        password = Base64.encode(password)
        if (this.userInfo.remember) {
          if (!getCookie(username)) {
            setCookie(username, password, 1000 * 60 * 60 * 2)
          }
        } else {
          if (getCookie(username)) {
            delCookie(username)
      ...
      searchPassword: function (username) {
        username = Base64.encode(username)
        if (getCookie(username)) {
          this.userInfo.remember = true
          this.userInfo.password = Base64.decode(getCookie(username))
        }
      }
    }
  }
...
.login {
  ...
  width: 100%;
  height: 100%;
  background: url("../../assets/login/icon/bg.png") no-repeat center center;
  ...
  .loginMap {
    ...
    width: 100%;
    height: 100%;
    /*background: url("../../assets/login/icon/Earth.png") no-repeat center center;*/
    /*background-size: cover;*/
  }
  .logo {
    width: (577/1920)*100%;
  ...
  .loginMain {
    ...
    position: absolute;
    z-index: 1000;
    box-shadow: rgba(0, 0, 0, 0.2) 0px 0px 8px 12px;
    ...
    .login_cont {
      ...
      .lable_cell {
        ...
        & span.icon_user {
          background: url("../../assets/login/login_icon.png") 10px 13px no-repeat;
        }
        & span.icon_pwd {
          background: url("../../assets/login/login_icon.png") -48px 14px no-repeat;
        }
      }
      & input.login_btn {
        width: 320px;
